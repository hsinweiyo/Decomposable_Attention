...
class DecomposableNLIModel(object):
    ...
    def train(self, session, train_dataset, valid_dataset, save_dir,
        ...
        sheet = book.add_sheet('hyperparameter')

        rate_des = 'Learning Rete'
        batch_des = 'Batch Size'
        num_des = 'Number Units'
        des_para = [rate_des, batch_des, num_des]
        params = [learning_rate, batch_size, self.num_units]
        for n, (u_desc, u) in enumerate(zip(des_para, params)):
            sheet.write(n, 0, u_desc)
            sheet.write(n, 1, u)
        epoch_des = 'Epoch'
        batch_des = 'Batch'
        vaild_des = 'Vaild_Loss'
        vaild_acc = 'Vaild_Accuracy'
        desc = [epoch_des, batch_des, vaild_des, vaild_acc]

        for n, v_desc in enumerate(desc):
            sheet.write(3, n, v_desc)
        print('Book gerenated!')
        ###
        ...
        for i in range(num_epochs):
            ...
            while batch_index < train_dataset.num_items:
                ...
                if batch_counter % report_interval == 0:
                    ...
                    valid_loss, valid_msg, valid_acc = self._run_on_validation(session,
                                                                    feeds)
                   ...
                   ### Add output variables
                        output_loss = '%s' % best_loss
                        output_acc = '%s' % valid_acc
                        variables = [i, batch_counter, output_loss, output_acc]
                        for n, v in enumerate(variables):
                            sheet.write(index, n, v)
                        index += 1
                  ###
                    logger.info(msg)
        book.save(save_dir+'/param.xls')
