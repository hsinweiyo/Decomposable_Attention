...
def create_dataset_SSQA(data_path,word_dict,label_dict,max_len1=None,max_len2=None):
    ...
    with codecs.open(data_path,'r',encoding='utf-8') as file:
        ...
        for data_id in range(int(len(lines)/8)):
            ...
            #print('raw_values:')
            #print(lines[data_id*8+1])
            raw_sentences1.append(lines[data_id*8+4])
            #print(lines[data_id*8+4].encode('utf-8'))
            raw_sentences2.append(lines[data_id*8+5])
            #print(lines[data_id*8+5].encode('utf-8'))
    def cleanSents(corpus):
    ...
    def cleanCon(corpus):
        cleaned_cons = [u'\u3002', u'\uff1f', u'\u3001',u'\u300c', u'\u300d', u'\uff1b']
        for c in cleaned_cons:
            corpus = [sent.replace(c, u'\uff0c') for sent in corpus]
        return corpus

    labels=cleanLabels(raw_labels)
    #print(labels[:5])
    raw_sentences1 = cleanCon(raw_sentences1)
    raw_sentences2 = cleanCon(raw_sentences2)
    tokens1=cleanText(raw_sentences1)#list of unicode strings
    tokens2=cleanText(raw_sentences2)
    # Clean all the useless mark
   
    output_sentences1 = cleanSents(raw_sentences1)
    output_sentences2 = cleanSents(raw_sentences2)
    #print(tokens1[:5])
    ...
    #pairs_SSQA = (raw_sentences1, raw_sentences2, labels)
    return RTEDataset(sentences1,sentences2,sizes1,sizes2,labels), raw_labels, output_sentences1, output_sentences2
